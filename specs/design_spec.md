...
- **주요 기능**:
  ...
  - 📝 **일기 카드**: 기기 대응 반응형 높이 (DIARY_CARD_HEIGHT: 가로비율 1.28 vs 화면높이 52% 중 최솟값) + 스티커 표시
...
- **역할**: 기분 + 일기 + 활동 + 스티커 + 사진 작성/수정
- **프리미엄 연동**: `isPremium` 상태에 따라 스티커 최대 개수 조절 (free: 5, premium: 99)
- **📱 툴바 (Toggle & Actions)**:
  - 입력 카드 및 페이지 네비게이션 아래에 가로로 배치
  - **🎞️ 노션 스타일 탭 스위치**: 라운딩(8px)과 보더가 들어간 깔끔한 사각형 탭 컨테이너 (`[ ✨ 스티커 | 📸 필름 ]`). 선택 시 #F1F1F0 배경색 적용
  - 스티커 탭 시 바텀시트(노션 스타일 카드)가 아래에 펼쳐짐
  - 필름 탭 시 바텀시트가 열려 화이트/블랙 폴라로이드 프레임을 선택할 수 있음
  - **🪄 다꾸 가챠**: 플로팅 독립 버튼 (기분별 스티커 2~3개 랜덤 자동 배치)
  - **⚙️ 관리**: 스티커 서랍 내부 카테고리 탭 우측 끝에 위치하는 아이콘 단독형 버튼 (배경/테두리 없음)
- **📷 사진 첨부**:
  - 필름 탭에서 프레임 종류(화이트/블랙) 선택 후 갤러리 오픈
  - 사진은 선택한 폴라로이드 프레임(하얀/검은 두꺼운 테두리 + 종이 그림자)으로 표시
  - 3층 레이어: TextInput → 사진(zIndex 5) → 스티커(zIndex 10)
- **🎨 배경지**: 현재 숨김 처리, 기본 흰색 고정
...
- **파일명**: `components/index.js`, `theme.js`, `stickers.js`, `DoodleStickers.js`, `ItStickers.js`, `DraggableSticker.js`, `DraggablePhoto.js`
- **MoodBar**: 게이지 차오름 애니메이션 적용 (`Animated.timing`, 600ms)
- **DraggablePhoto**: 폴라로이드 프레임 사진 (드래그+회전 지원, 로직/뷰/스타일 분리)
