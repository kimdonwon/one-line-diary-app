...
* **시각적 특징**:
  ...
  * **다꾸 전용 하단 탭바**: 일기 쓰기 화면에서는 하단 네비게이션 탭의 버튼 구성이 `[스티커 | 카메라 | 완료 | 텍스트 | 설정]`으로 변경되어 다이어리 꾸미기에 최적화된 동선을 제공합니다.
  ...
  * **Floating Glass Island & Seamless Morphing Dock**: 하단 도구 패널(텍스트, 사진, 스티커)은 반투명 블러 효과(`expo-blur`의 BlurView)가 적용된 단일 컨테이너로 통합되었으며, 탭 전환 시 창이 닫히지 않고 내부에서 매끄럽게 교체(LayoutAnimation)되어 MZ세대 트렌드에 맞는 플로팅 디자인과 심리스 트랜지션을 제공합니다.
