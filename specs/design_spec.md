...
* **시각적 특징**:
  ...
  * **인디케이터 색상 동기화**: 페이지 인디케이터(Dots)가 위치한 상단 영역의 배경색을 현재 캔버스의 배경색과 100% 일치시키고 여유로운 상하 여백(`paddingVertical: 8pt`)을 부여하여, 카드 전체가 파편화되지 않고 하나의 완성된 테마처럼 느껴지도록 시각적 완성도를 높임. 우측 구석에는 페이지 삭제가 가능한 미니멀한 버튼(✕, fontSize 11pt, #D1D1D1, right: 10pt)을 배치해 기능성과 청결함을 동시 확보. *(v2.3 추가)*
