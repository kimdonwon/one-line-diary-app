...
- **해결**:
    1. `InteractionManager`를 이용해 화면 전환 완료 후 비동기 지연 파싱(Lazy Parsing) 적용.
    2. 글로벌 캐시(`parsedMoodCache`) 도입으로 중복 파싱 차단.
    3. `isPaused` SharedValue를 적용하여 비활성 시 Worklet 연산 즉시 중단.
- **파일**: `src/components/SkiaConfettiEffect.js`, `src/hooks/useBentoBoard.js`

## 10. WriteScreen 기분 선택 바텀시트 미출력 (2026-03-31)

- **증상**: 일기 쓰기 진입 시나 이모지 클릭 시 바텀시트가 나타나지 않음.
- **원인**:
    1. **Android Elevation**: `RNModal` 제거 후 `Animated.View`로 교체하면서 `elevation` 누락으로 캔버스 뒤에 가려짐.
    2. **Native Driver Sync**: 뷰가 마운트되기 전 애니메이션이 시작되어 드라이버 연결 실패.
- **해결**:
    1. 바텀시트 및 백드롭에 `elevation: 10000`을 부여하여 렌더링 뎁스 최상단 고정.
    2. `setTimeout(..., 16)`을 적용하여 View가 React Tree에 부착된 후 애니메이션 킥오프되도록 보정.
...
## 3. 선택 상태 드래그 시 페이지 이동 간섭 해결 (2026-03-13)

- **증상**: 아이템이 선택된 상태에서 빠르게 드래그를 시작할 때, 좌우 페이지 슬라이드(FlatList 스와이프)가 함께 발생하여 드래그가 끊기거나 페이지가 넘어가는 현상 발생.
- **원인**: `onMoveShouldSetPanResponderCapture`에서 제스처를 가로채기 전 1픽셀의 최소 이동 거리(`moveDist > 1`) 조건을 두었는데, 빠른 제스처 시 이 짧은 찰나에 페이징 스크롤 엔진이 먼저 발동됨.
- **해결**: `useDraggable.js`의 캡처 단계에서 이미 선택된 아이템(`effectiveSelected`)인 경우 거리 체크 없이 즉시 `true`를 반환하도록 수정하여 부모 스크롤이 발동될 틈을 주지 않고 제스처를 선점함.
