...
- **위치**: `WriteScreen.view.js` (멀티페이지 FlatList)
- **트리거**: 작성 화면 최초 진입(마운트) 후 `pageCardWidth`가 계산되었을 때 (최초 1회 제한)
- **동작 방식**:
  1. `24` 픽셀 오프셋으로 부드럽게 이동 (오른쪽으로 밀림)
  2. 400ms 후 원래 위치(`0`)로 복귀
- **목적**: 가로 스와이프를 통해 페이지를 무한으로 추가할 수 있는 "엣지 풀 트리거"의 존재를 직관적으로 상기시킴.
