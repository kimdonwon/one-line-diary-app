...
이 스킬은 스티커 카테고리(팩) 데이터를 렌더링하고 사용자에게 보여주는 UI/로직 규칙을 정의합니다. 앱에서 스티커 팩 정보를 표현할 때는 항상 이 스킬을 따릅니다. 하나의 스티커팩은 18개의 스티커로 구성되어 있습니다.
...
## 📏 5. 일기 박스 내 스티커 크기 표준화
- 사용자가 일기 작성 박스에 배치하는 스티커(DraggableSticker)는 일기 텍스트와의 시각적 조화를 위해 다음 표준 크기를 준수합니다.
- **SVG 그래픽 스티커**: `size={29}` (컴포넌트 props 기준)
- **이모지 텍스트 스티커**: `fontSize: 26`, `lineHeight: 31` (스타일 시트 기준)
- 새로운 스티커 컴포넌트를 만들거나 기존 스티커의 렌더링 로직을 수정할 때, 항상 위 수치를 기본값으로 적용하여 일관된 사용자 경험을 제공합니다.
